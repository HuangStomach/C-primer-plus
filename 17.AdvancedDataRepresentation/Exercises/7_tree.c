...
static Pair SeekItem(const Item * i1, const Tree * ptree);
...
bool AddItem(const Item * pi, Tree * ptree) {
    ...
    if (pair.child != NULL) {
        pair.child->item.count++;
        return false;
    }
    
    new_node = MakeNode(pi);
    if (new_node == NULL) {
        fprintf(stderr, "Couldnot create node\n");
        return false;
    }
...
int InTree(const Item * pi, const Tree * ptree) {
    Pair pair = SeekItem(pi, ptree);
    return (pair.child == NULL) ? 0 : pair.child->item.count;
}
...
bool DeleteItem(const Item * pi, Tree * ptree) {
    ...
    else if (look.parent->left == look.child) DeleteNode(&look.parent->right);
